...
public class Networking {
    ...
    public static MoPubRequestQueue getRequestQueue(@NonNull final Context context) {
        ...
        MoPubLog.e("Networking.getRequestQueue");
        // Double-check locking to initialize.
        if (requestQueue == null) {
            MoPubLog.d("Before enter getRequestQueue synchronized (Networking.class)");
            synchronized (Networking.class) {
                MoPubLog.e("Entered getRequestQueue synchronized (Networking.class)");
                ...
                if (requestQueue == null) {
                    ...
                    MoPubLog.d("Before Networking.getUserAgent");
                    final String userAgent = Networking.getUserAgent(
                            context.getApplicationContext());
                    MoPubLog.d("After Networking.getUserAgent " + userAgent);
                ...
                MoPubLog.e("Left synchronized (Networking.class)");
            }
        }
    ...
    public static String getUserAgent(@NonNull Context context) {
        Preconditions.checkNotNull(context);
        MoPubLog.e("Networking.getUserAgent");
        String userAgent = sUserAgent;
        if (userAgent == null) {
            MoPubLog.d("Before enter getUserAgent synchronized (Networking.class)");
            synchronized (Networking.class) {
                MoPubLog.e("Entered getUserAgent synchronized (Networking.class)");
                userAgent = sUserAgent;
                if (userAgent == null) {
                    try {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                            MoPubLog.d("Before WebSettings.getDefaultUserAgent");
                            userAgent = WebSettings.getDefaultUserAgent(context);
                            MoPubLog.d("After WebSettings.getDefaultUserAgent " + userAgent);
